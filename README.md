...
#### goose.engine.data
raw data for model training can be supplied both directly through Football-Goose and/or externally. Football-Goose provides wrapping classes around the [soccerdata](https://github.com/probberechts/soccerdata) project that allows for easily pulling key datasets (standings, schedule, results, etc.) and translating them to a common set of team names (i.e. Brighton vs. Brighton and Hove Ablion vs. B and H Albion, etc.)
