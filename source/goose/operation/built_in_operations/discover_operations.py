...
@discover_operations.operation("ops-tree", automatically_supports_cli = True, description = "Displays tree of all available operations")
def operations_tree():
    print(f"{50 * '-'}")
...
@discover_operations.operation("models", automatically_supports_cli = True, description = "Displays list of accessible models")
def models():
    registry.models_registry.display()

# prints list of accessible model definitions
# goose discover model-defs 
@discover_operations.operation("model-defs", automatically_supports_cli = True, description = "Displays list of accessible model definitions")
def model_defs():
    registry.model_definitions_registry.display()

# prints display of accesible data types and sources
@discover_operations.operation("data-types", automatically_supports_cli = True, description = "Displays all accessible data types and sources")
def data_types():
    registry.data_types_registry.display()

# prints all structures successfully discovered by goose
# goose discover all
@discover_operations.operation("all", automatically_supports_cli = True, description = "Displays all structures discovered by Football-Goose")
def all():
    operations_tree()
    models()
    model_defs()
