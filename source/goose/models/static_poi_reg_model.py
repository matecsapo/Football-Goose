...
class Static_Poi_Reg_Model(Goose_Model):
    ...
    def Save_Model(self):
        ...
        # model evaluation statistics
        with open(f"{folder}/eval_statistics.json", "w") as f:
            json.dump(self.Model_Evals, f, indent=4)
        # model identification (model type + timestamp saved)
        with open(f"{folder}/model_identification.json", "w") as f:
            json.dump({
                "Model Type" : "Static Poisson Regression Model",
    ...
    def Predict_Game(self, game : Game):
        ...
        # return report
        return Game_Prediction(game, home_xg, away_xg, home, away, prob_home_win, prob_away_win, prob_draw)
